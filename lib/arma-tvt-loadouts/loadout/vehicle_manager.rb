  ...
  module TvT
    module Loadouts
      class VehicleManager
        ...
        def list(json)
          @logger.info('listing vehicles and static objects...')
          vehicles = []
          json[:Mission][:Entities].drop(1).select {|s| s[1][:dataType].upcase == 'OBJECT' && s[1][:side].upcase == 'EMPTY'}.each { |object|
            vehicles << object[1][:type]
          }
          vehicles.sort.uniq
        end
          ...
          json[:Mission][:Entities].drop(1).select {|s| s[1][:dataType] == 'Object'}.each { |group|
            if %w'EMPTY'.include? (group[1][:side].upcase)
              ...
              unless vehicleSide.nil? or is_ignored(group[1][:Attributes])
                ...
                @logger.info("Unprocessed vehicle, class: '#{group[1][:type]}'")
        ...
        def get_cargo_items(type, side)
          ...
          c = config["vehicles-#{side}"][:common]['cargo'].merge(config["vehicles-#{side}"][type]['cargo']) { |key, oldValue, newValue|
            oldValue ||= []
            newValue ||= []
            oldValue + newValue
          }
        ...
        def is_ignored(attributes)
          unless attributes.nil? or attributes[:init].nil?
            return attributes[:init] =~ /this\s*setVariable\s*\\\[\s*\['"\]{1}tvtmtk_ignore\['"\]\s*,\s*1\s*\\\]\s*;/i
          end
          false
        end
        ...
        def get_vehicle_side(classname)
          %w'bluefor redfor greenfor civfor'.each { |side|
            config["vehicles-#{side}"].each { |key, value|
              return side,key if value.is_a?(Hash) and !value['classes'].nil? and value['classes'].include?(classname)
            }
          }
          return nil,nil
        end
