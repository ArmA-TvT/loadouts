  ...
  module TvT
    module Loadouts
      class InfantryManager
        ...
        #include ::ArmA::TvT::Loadouts::Config
        include ::ArmA::TvT::Loadouts::Mission::Sided

        attr_accessor :config

        def initialize
          @config_manager = ArmA::TvT::Loadouts::Config::Manager.instance
          @logger = @config_manager.logger
        end

        def list(json)
          @logger.info('listing infantry units...')
          units = { :bluefor => [], :redfor => [], greenfor: [], civfor: [] }
          json[:Mission][:Entities].drop(1).select {|s| s[1][:dataType].upcase == 'GROUP'}.each { |group|
            if %w'EAST WEST INDEPENDENT CIVILIAN'.include? (group[1][:side].upcase)
              side = self.get_color(group[1][:side])
              group[1][:Entities].drop(1).select {|s| s[1][:dataType] == 'Object'}.each { |u|
                units[side] << u[1][:type]
                puts u[1][:type]
              }
            else
              raise Error.new "unsupported side '#{group[1][:side]}'"
            end
          }
          units.each_value { |v| v.sort.uniq}
        end

        def generate(json)
          logger.info('inserting loadouts...')
          json[:Mission][:Entities].drop(1).select {|s| s[1][:dataType] == 'Group'}.each { |group|
            if %w'EAST WEST RESISTANCE CIVILIAN'.include? (group[1][:side].upcase)
              group_callsign = ''
              ...
              group[1][:Entities].drop(1).select {|s| s[1][:dataType] == 'Object'}.each { |u|
                ...
                unit[:Attributes][:isPlayable] = 1 if config[:all_playable] and (!unit[:Attributes].key?(:isPlayer) or unit[:Attributes][:isPlayer] < 1)
                ...
                unless unit[:Attributes].key?(:init) and unit[:Attributes][:init].include?('this setVariable [""tvtmtk_ignore"", 1];')
        ...
        def get_face(unit, mtkClass)
          loadouts = config["infantry-#{get_color(unit[:side])}".to_sym]
        ...
        def get_loadouts(mtkClassName, side)
          ...
          res = Hash.new
          side_gears = config["infantry-#{get_color(side)}".to_sym]
        ...
        def add_gears(mtkClassName, side)
          ...
          gears = config["infantry-#{get_color(side)}".to_sym]
        ...
        def add_weapons(mtkClassName, side)
          ...
          weapons = config["weapons-#{get_color(side)}".to_sym]
          ...
          w.each { |key,value|
            ...
            when 'weapon'
              ...
              value.each { |weapon|
                ...
                if weapon.is_a?(Hash)
                  if weapon.key?('ammo')
                    res += "[this,1,\"#{weapon['ammo']}\"] call tvtmtk_fnc_addItem;" unless weapon['ammo'].nil?
                  end
                  ...
                  #   case category
                  #     when 'ammo'
                  #       res += "[this,1,\"#{val}\"] call tvtmtk_fnc_addItem;" unless val.nil?
                  #     when 'items'
                  #       val.each{ |item|
                  #         action = index < 1 ? 'addPrimaryWeaponItem' : 'addSecondaryWeaponItem'
                  #         res += "this #{action} \"#{item}\";"
        ...
        def add_quantified_item(container, item)
          ...
          return "[this,#{quantity},\"#{itemClass}\"] call tvtmtk_fnc_#{container};"
        end
        ...
        def get_MTK_class(arma3ClassName, side)
          ...
          klasses = config["classes-#{get_color(side)}".to_sym]
        ...
        def get_classes
          ...
          result[:bluefor] = config['classes-bluefor'.to_sym]
          result[:redfor] = config['classes-redfor'.to_sym]
          result
        end
