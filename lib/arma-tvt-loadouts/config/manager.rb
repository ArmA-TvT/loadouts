      ...
      module Config
        class Manager
          ...
          def validate_path(path)
            ...
            logger.debug("Checking that '#{path}' contains 'arma-tvt-loadouts-loadouts' sub-directory")
            raise "#{prefix} '#{File.absolute_path(File.join(path, 'arma-tvt-loadouts-loadouts'))}' TVTMTK data missing" unless Dir.exist?(File.join(path, 'arma-tvt-loadouts-loadouts'))
          end
